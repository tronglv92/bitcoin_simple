...
const (
	...
	BLOCKCHAIN_PORT_RANGE_START      = 5100
	BLOCKCHAIN_PORT_RANGE_END        = 5103
...
func (bc *Blockchain) CreateTransaction(sender string, recipient string, value float32,
	...
	if isTransacted {
		for _, n := range bc.neighbors {
			//convert an integer to a hex string with a fixed length of 64 characters,
...
func (bc *Blockchain) ValidChain(chain []*Block) bool {
	...
	for currentIndex < len(chain) {
		...
		if b.previousHash != preBlock.Hash() {
			log.Println("b.previousHash == ", b.previousHash)
			log.Println("preBlock.Hash() == ", preBlock.Hash())
			return false
		}

		if !bc.ValidProof(b.Nonce(), b.PreviousHash(), b.Transactions(), MINING_DIFFICULTY) {
			log.Println("Not valid proof")
			return false
		}
...
func (bc *Blockchain) ResolveConflicts() bool {
	...
	for _, n := range bc.neighbors {
		...
		if resp.StatusCode == 200 {
			log.Println("get chain success == ", resp.Body)
			...
			log.Println("bc.ValidChain(chain) == ", bc.ValidChain(chain))
